...
nmds_dis_gene <- wascores(nmds_dis$points, data)
...
merged=merge(nmds_dis,zhushi,by="row.names",all.x=TRUE)

custom_colors <- c("#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd")

ggplot(data = merged, aes(NMDS1, NMDS2)) +
  geom_point(size = 2, aes(color = factor(level), shape = factor(level))) +
  stat_ellipse(aes(fill = factor(level)), geom = 'polygon', level = 0.95, alpha = 0.1, show.legend = FALSE) +
